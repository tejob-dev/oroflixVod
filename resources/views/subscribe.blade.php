...
<section id="main-wrapper" class="main-wrapper user-account-section stripe-content">
      ...
      <div class="col-md-8 col-sm-12">
       &nbsp;
        <div class="col-md-4 col-sm-4 col-xs-12"> <!-- required for floating -->
          ...
          <ul class="nav nav-tabs tabs-left sideways">
            ...
            <!-- end omise -->
             ...
             <!-- worlday tab -->
            @if (isset($worldpay_payment) && $worldpay_payment == 1 && in_array('worldpay',$currency_payments))
              <li class=""><a href="#worldpay" data-toggle="tab">{{ __('Check out With') }} WorldPay</a></li>
            @endif
            <!-- end worlday -->


             <!-- squarepay tab -->
            @if (isset($squarepay_payment) && $squarepay_payment == 1 && in_array('squarepay',$currency_payments))
              <li class=""><a href="#square" data-toggle="tab">{{ __('Check out With') }} Squarepay</a></li>
            @endif
            <!-- end squarepay -->
        ...
        <div class="col-md-8 col-sm-8 col-xs-12">
          <!-- Tab panes -->
          &nbsp;
          <h4 class="heading"><a href="{{url('account')}}">{{ __('Pay') }} &nbsp;<i class="{{ $currency_symbol }}"></i> {{ $plan->amount }}</a></h4>
          <hr/>
        @php
  ...
  <!-- worldpay tab -->
    ...
    <h3>{{__('staticwords.Pay')}}</h3>
    <div class="row">

        <div class="col-md-6">
            <div class="form-container active">
                <form action="{{ route("worldpay.payment")  }}" id="worldpaypaymentForm" method="post">
                  <script src="https://cdn.worldpay.com/v1/worldpay.js"></script>
                    ...
                    <input type="hidden" name="amount" value="{{ strip_tags($plan->amount) }}">
                    <hr>
                    <div class="form-group">
...
<div class="tab-pane" id="square">
     @if(env('SQUARE_PAY_LOCATION_ID') != NULL && env('SQUARE_ACCESS_TOKEN') != NULL && env('SQUARE_APPLICATION_ID') != NULL)
        ...
        <form id="sqp-payment-form" method="POST" action="{{ route('squarepay.front.payment') }}">
            ...
            <!-- Submit Button -->
            <div class="form-group">
                 <button id="card-button1" class="btn btn-primary" title="checkout" type="button">{{ __('Proceed') }}</button>
            </div>
...
<script src="//js.stripe.com/v3/"></script> <!-- stripe script -->
 ...
 <!-- Squarepay payment tab content end -->
    ...
    <script>
        const appId      = @json(config('square.SQUARE_APPLICATION_ID'));
       
        const locationId = @json(config('square.SQUARE_PAY_LOCATION_ID')); 
