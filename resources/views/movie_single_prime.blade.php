...
@if(isset($movie) && is_object($movie) && isset($movie->title) && isset($movie->detail) && isset($movie->thumbnail)) 
 @section('custom-meta')
...
@elseif($movie->tvseries != null && $movie->tvseries->poster != null)
    <div class="background-main-poster" style="background-image: url('{{url('images/tvseries/posters/'.$movie->tvseries->poster)}}');">
@else
        ...
        @if($configs->comments == 1)
          ...
          <div role="tabpanel" class="tab-pane fade" id="postcomment">
              <div style="width: 90%;color:#B1B1B1;" class=" " >
                  <h3 class="author-heading post-comment-heading">{{__('Post Comment')}}:</h3><br/>
  ...
  @if($configs->comments == 1)
    ...
    <!-- Tab panes -->
      <div class="tab-content">
        ...
        <div role="tabpanel" class="tab-pane fade" id="postcomment">
            <div style="width: 90%;color:#B1B1B1;" class=" " >
                <h3 class="post-comment-heading">{{__('Post Comment')}}:</h3><br/>
...
$currentPage = new Share();
$currentPageData = $currentPage->currentPage();

                        if($currentPage) {
                            $facebookLinks = $currentPage->facebook()->getRawLinks();
                            $twitterLinks = $currentPage->twitter()->getRawLinks();
                            $telegramLinks = $currentPage->telegram()->getRawLinks();
                            $whatsappLinks = $currentPage->whatsapp()->getRawLinks();
                        } else {
                            $facebookLinks = null;
                            $twitterLinks = null;
                            $telegramLinks = null;
                    ...
                    @endif
                    <?php
if (is_array($twitterLinks)) {
    foreach ($twitterLinks as $link) {
        echo $link . '<br>'; // Output each element with a line break
    }
} else {
    echo $twitterLinks; // Echo the string directly if it's not an array
}
?>
 <?php
if (is_array($telegramLinks)) {
    foreach ($telegramLinks as $link) {
        echo $link . '<br>'; // Output each element with a line break
    }
} else {
    echo $telegramLinks; // Echo the string directly if it's not an array
}
?>
<?php
if (is_array($whatsappLinks)) {
    foreach ($whatsappLinks as $link) {
        echo $link . '<br>'; // Output each element with a line break
    }
} else {
    echo $whatsappLinks; // Echo the string directly if it's not an array
}
?>
