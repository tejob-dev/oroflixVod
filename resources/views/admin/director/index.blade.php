      ...
      <div class="card-header movie-create-heading">
        <div class="row">
          <div class="col-lg-4 col-md-4 col-4">
            <h5 class="card-title">{{ __('All Directors') }}
                 <input class="grand_selectallm ml-3" type="checkbox">
                                        {{__('Select All') }}
                                      </h5>
          </div>
          <div class="col-lg-8 col-md-8 col-8 text-right mb-2">
            @can('director.delete')
            <a href="{{route('directors.create')}}" class="btn btn-primary-rgba mr-2" title="{{ __('Add Director') }}"><i
                class="feather icon-plus mr-2"></i>{{ __('Add Director') }} </a>
            @endcan

            <button type="button" class="btn btn-success-rgba mr-2 " data-toggle="modal"
            data-target=".bd-example-modal-lg" title="{{ __('Import Director') }}"><i class="fa fa-file-excel-o mr-2"></i> {{ __('Import Director') }} </button>
                      ...
                      <div class="modal-header">
                          ...
                          <div class="col-md-12">
                              <div class="card-header">
                                  <a href="{{ url('files/Directors.xlsx') }}" class="float-right btn btn-success-rgba mr-2" title="{{ __('Download Example xls/csv File') }}"><i
                    ...
                    <div class="row">
                      ...
                      @foreach($directors as $item)
                        ...
                        <div class="col-lg-3 col-md-6 col-sm-6 col-xs-12">
                            ...
                            <div class="card">
                              @if($src != NULL)
                              <a href="{{url('video/detail/director_search', $item->slug)}}" target="__blank" title="{{$item->name}}"><img src="{{$src}}" class="card-img-top" alt="{{$item->name}}"></a>
                              @endif
                                <div class="overlay-bg"></div>
                                <div class="dropdown card-dropdown">
                                    <a class="btn btn-round btn-outline-primary pull-right dropdown-toggle" type="button" id="dropdownMenuButton-{{$item->id}}" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" title="Settings"><i class="feather icon-more-vertical-"></i></a>
                                    <div class="dropdown-menu pull-right" aria-labelledby="dropdownMenuButton-{{$item->id}}">
                                      @can('director.view')
                                        <a class="dropdown-item" href="{{url('video/detail/director_search', $item->slug)}}" target="__blank" title="{{__('View')}}"><i class="feather icon-monitor mr-2"></i> {{__('View')}}</a> 
                                      @endcan
                                      @can('director.edit')                                       
                                        <a class="dropdown-item" href="{{ route('directors.edit', $item->id)}}" title="{{ __('Edit') }}"><i class="feather icon-edit mr-2"></i> {{__('Edit')}}</a>
                                      @endcan
                                      @can('director.delete')                                      
                                ...
                                <div class="card-header">
                                  <h5 class="card-title"><a href="{{url('video/detail/director_search', $item->slug)}}" target="__blank" title="{{$item->name}}">{{$item->name}}</a></h5><br>
                                </div>
                                <div class="card-body director-body">
