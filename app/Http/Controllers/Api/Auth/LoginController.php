    ...
    public function sociallogin(Request $request)
{
    ...
    $authUser = User::where('email', $request->email)
                    ->orWhere($providerField, $request->code)
                    ->first();
    ...
    if ($authUser) {
        ...
        if ($authUser->is_blocked == 1) {
            ...
            if (Hash::check('password', $authUser->password)) {
                // Issue token using createToken method
    ...
    public function refresh(Request $request)
{
    ...
    $newAccessToken = Auth::guard('api')->user()->createToken('Personal Access Token')->accessToken;
